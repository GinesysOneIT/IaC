variable "vm_name_state_01" {
  type    = string
  default = "vm-erp-repstate-prod-jiw-01"
}



resource "azurerm_network_interface" "state-nic-01" {
  location            = "jioindiawest"
  name                = "nic-${var.vm_name_state_01}"
...
resource "azurerm_virtual_machine" "vm-state-01" {
  location              = "jioindiawest"
  name                  = var.vm_name_state_01
  network_interface_ids = [azurerm_network_interface.state-nic-01.id]
  resource_group_name   = "rg-report-prod-jiw"
  tags = {
    BACode      = "None"
  ...
  storage_os_disk {
    ...
    managed_disk_id = azurerm_managed_disk.state-disk-01.id
    name            = "disk-os_${var.vm_name_state_01}"
    os_type         = "Windows"
  }
}
