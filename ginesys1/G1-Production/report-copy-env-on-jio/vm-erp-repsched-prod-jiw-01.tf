variable "vm_name_sched_01" {
  type    = string
  default = "vm-erp-repsched-prod-jiw-01"
}



resource "azurerm_network_interface" "sched-nic-01" {
  location            = "jioindiawest"
  name                = "nic-${var.vm_name_sched_01}"
...
resource "azurerm_virtual_machine" "vm-sched-01" {
  location              = "jioindiawest"
  name                  = var.vm_name_sched_01
  network_interface_ids = [azurerm_network_interface.sched-nic-01.id]
  resource_group_name   = "rg-report-prod-jiw"

  os_profile {
    computer_name  = "rep-sched-01"
  ...
  storage_os_disk {
    ...
    name          = "disk-os-${var.vm_name_sched_01}"
    disk_size_gb  = 128
    os_type       = "Windows"
  }
